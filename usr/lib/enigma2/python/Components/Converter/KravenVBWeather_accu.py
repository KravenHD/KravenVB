...
from Plugins.Extensions.KravenVB import ping
...
URL = 'http://dataservice.accuweather.com/forecasts/v1/daily/5day/' + str(config.plugins.KravenVB.weather_accu_id.value) + '?apikey=' + str(config.plugins.KravenVB.weather_accu_apikey.value) + '&language=' + str(config.plugins.KravenVB.weather_language.value + '&details=true&metric=true')
URL2 = 'http://dataservice.accuweather.com/currentconditions/v1/' + str(config.plugins.KravenVB.weather_accu_id.value) + '?apikey=' + str(config.plugins.KravenVB.weather_accu_apikey.value) + '&language=' + str(config.plugins.KravenVB.weather_language.value + '&details=true')

WEATHER_DATA1 = None
WEATHER_DATA2 = None
WEATHER_LOAD = True

class KravenVBWeather_accu(Poll, Converter, object):
	...
	def getText(self):
		...
		if self.what == 'DayTemp':
			...
			self.info = str(config.plugins.KravenVB.weather_foundcity.getValue())
	...
	def get_Data(self):
		...
		if WEATHER_LOAD == True:
			try:
				...
				pass
			timeout = max(15,int(config.plugins.KravenVB.refreshInterval.value)) * 1000.0 * 60.0
