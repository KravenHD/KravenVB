...
class KravenVBColorSelection(ConfigListScreen, Screen):

	if DESKTOP_WIDTH <= 1280:
	  skin = """
<screen name="KravenVBColorSelection" position="0,0" size="1280,720" flags="wfNoBorder" backgroundColor="#FF000000">
  ...
  <widget backgroundColor="#00000000" source="Title" render="Label" font="Regular;28" foregroundColor="#00ffffff" position="844,260" size="150,36" halign="right" transparent="1" valign="center">
    <convert type="KravenVBColorInfo">Kraven</convert>
  </widget>
  ...
  <ePixmap backgroundColor="#00000000" pixmap="KravenVB/buttons/key_red1.png" position="281,442" size="200,5" alphatest="blend" />
  <ePixmap backgroundColor="#00000000" pixmap="KravenVB/buttons/key_green1.png" position="531,442" size="200,5" alphatest="blend" />
  <eLabel backgroundColor="#00000000" position="266,250" size="748,220" transparent="0" zPosition="-1" />
</screen>
"""
	else:
	  skin = """
<screen name="KravenVBColorSelection" position="0,0" size="1920,1080" flags="wfNoBorder" backgroundColor="#FF000000">
...
"""
	...
	def __init__(self, session, title = "", color = ""):
		...
		config.plugins.KravenVB.SelfColorR.value = int(self.newcolor[0:2], 16)
		config.plugins.KravenVB.SelfColorG.value = int(self.newcolor[2:4], 16)
		config.plugins.KravenVB.SelfColorB.value = int(self.newcolor[4:6], 16)
	...
	def colorselectionlist(self):
		list = []
		list.append(getConfigListEntry(_("red"), config.plugins.KravenVB.SelfColorR))
		list.append(getConfigListEntry(_("green"), config.plugins.KravenVB.SelfColorG))
		list.append(getConfigListEntry(_("blue"), config.plugins.KravenVB.SelfColorB))

		self["config"].list = list
		self["config"].l.setList(list)

		self.showColor(self.RGB(int(config.plugins.KravenVB.SelfColorR.value), int(config.plugins.KravenVB.SelfColorG.value), int(config.plugins.KravenVB.SelfColorB.value)))
		self.newcolor=str(hex(config.plugins.KravenVB.SelfColorR.value)[2:4]).zfill(2) + str(hex(config.plugins.KravenVB.SelfColorG.value)[2:4]).zfill(2) + str(hex(config.plugins.KravenVB.SelfColorB.value)[2:4]).zfill(2)
